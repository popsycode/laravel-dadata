### Copy of removed *fomvasss/laravel-dadata*
Changed namespace Fomvasss/* to Popsy/*
...
Запустить:
```bash
composer require "popsy/laravel-dadata"
```
---
__Для Laravel < 5.5:__
Зарегистрировать service-provider в config/app.php:
```php
  Popsy\Dadata\DadataServiceProvider::class,
```
...
Опубликовать конфиг: 
```bash
php artisan vendor:publish --provider="Popsy\Dadata\DadataServiceProvider"
```
...
```php
use Popsy\Dadata\Facades\DadataSuggest;
```
...
Добавить в клас фасад:
```php
use Popsy\Dadata\Facades\DadataClean;
```
Использовать методы: 
```php
